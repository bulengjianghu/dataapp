...
| 术语 | 定义 |
|---|---|
| 表单定义 FormDefinition | 一个业务表单的逻辑身份，不直接承载可运行结构 |
| 表单草稿 FormDraft | 表单当前可编辑版本 |
| 表单版本 FormVersion | 一次发布形成的不可变快照 |
| 字段 Field | 表单中的基础输入单元 |
...
| 表名 | 说明 | 关键字段 |
|---|---|---|
| `form_definition` | 表单主实体 | `id` `form_code` `name` `status` `current_version_id` |
| `form_draft` | 当前草稿 | `form_id` `schema_json` `updated_by` `updated_at` |
| `form_version` | 发布快照 | `id` `form_id` `version_no` `schema_json` `published_at` |
